import React from "react";
import type { Observation } from "../../../../backend/src/models";

interface ObservationItemProps extends React.HTMLAttributes<HTMLDivElement> {
  observation: Observation;
}

export default function ObservationItem({
  observation,
  className = "",
  ...props
}: ObservationItemProps) {
  return (
    <div
      className={`
        mt-2 p-3 group/item
        max-h-[85px] max-w-full overflow-hidden
        transition-all duration-300 ease-in-out hover:max-h-[250px]
        border-b border-line-2 rounded-sm bg-bg-2/50
        hover:border-cy/40 hover:shadow-[0_0_10px_rgba(0,243,255,0.05)]
        ${className}
      `}
      {...props}
    >
      {/* Header */}
      <div className="flex justify-between items-start mb-1">
        <div className="font-mono text-cy uppercase tracking-widest text-[10px] font-bold">
          {observation.type}
        </div>
        <div className="text-[9px] opacity-30 font-mono">
          {observation.source}
        </div>
      </div>

      {/* Payload */}
      <div className="text-[12px] leading-snug text-fg-muted mb-2">
        <pre className="whitespace-pre-wrap">
          {JSON.stringify(observation.payload, null, 2)}
        </pre>
      </div>

      {/* Footer */}
      <div className="flex justify-between items-center pt-2 border-t border-line-2/50 opacity-0 group-hover/item:opacity-100 transition-opacity duration-300">
        <span className="text-[9px] font-mono opacity-40">
          {observation.timestamp
            ? new Date(observation.timestamp).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              })
            : "no timestamp"}
        </span>

        <span className="text-[9px] font-mono text-cy uppercase tracking-tighter">
          OBSERVATION
        </span>
      </div>
    </div>
  );
}
